  ...
  bool is_loan)
{
  ...
  *taken = false;
  ...
  dds_ReturnCode_t ret;
  if(is_loan) {
    ret = dds_DataReader_take_next_loaned_sample(topic_reader, ros_message, &sample_info);
  } else {
    ret = dds_DataReader_take_next_sample(topic_reader, *ros_message, reinterpret_cast<dds_SampleInfo *>(&sample_info));
  }
  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  *taken = 0;
  ...
  for(size_t i = 0; i < count; i++) {
    ...
    dds_ReturnCode_t ret = dds_DataReader_take_next_sample(topic_reader, message_sequence->data[*taken], &sample_info);
    if (ret == dds_RETCODE_NO_DATA) {
      RCUTILS_LOG_DEBUG_NAMED(
          "rmw_gurumdds_cpp", "No data on topic %s", topic_name);
      break;
  ...
  void * loaned_message)
{
  ...
  dds_DataReader_return_loaned_sample(topic_reader, loaned_message);
  return RMW_RET_OK;
}
}  // extern "C"
